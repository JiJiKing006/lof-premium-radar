...
小程序接口默认使用线上 `https://jijiking.top`，接口路径为 `/api/funds/quotes`、`/api/funds/:code`、`/api/funds/:code/history`。真机预览不依赖本机 `127.0.0.1`，需要在微信公众平台把 `https://jijiking.top` 配置为 request 合法域名。
