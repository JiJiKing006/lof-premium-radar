...
async function fetchFreshQuotes(category, cacheKey) {
  ...
  const errors = [];
  const referenceRows = category === 'LOF' ? await loadLofReferenceRows(errors) : [];
...
async function loadLofReferenceRows(errors) {
  const cached = cache.get(LOF_REFERENCE_CACHE_KEY);
  if (cached) return cached;

  const startedAt = Date.now();
  try {
    const rows = await fetchPalmmicroLofReferenceRows();
    ...
    return cache.set(LOF_REFERENCE_CACHE_KEY, referenceRows, cacheTtl.fundList);
  } catch (error) {
